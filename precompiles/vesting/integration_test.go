	...
	Context("to fund a clawback vesting account", func() {
		...
		for _, callType := range callTypes {
			...
			BeforeEach(func() {
				...
				vestingAccountKey = s.keyring.GetKey(1)
				...
				if callType.directCall == false {
					...
					auths, err := s.grpcHandler.GetAuthorizations(sdk.AccAddress(contractAddr.Bytes()).String(), funderKey.AccAddr.String())
					Expect(err).To(BeNil())
			...
			It(fmt.Sprintf("should fund the vesting when defining only lockup (%s)", callType.name), func() { //nolint:dupl
				...
				err = s.factory.CreateClawbackVestingAccount(vestingAccountKey.Priv, funderKey.AccAddr, false)
				Expect(err).To(BeNil())
				Expect(s.network.NextBlock()).To(BeNil())

				vestAccInitialBal, err := s.grpcHandler.GetBalance(vestingAccountKey.AccAddr, s.bondDenom)
				Expect(err).To(BeNil())
				...
				s.ExpectVestingAccount(vestingAccountKey.Addr, defaultPeriods, instantPeriods)

				vestCoinsAmt := math.NewIntFromBigInt(defaultPeriods[0].Amount[0].Amount)
				balRes, err := s.grpcHandler.GetBalance(vestingAccountKey.Addr.Bytes(), s.bondDenom)
				Expect(err).To(BeNil())
				vestAccFinalBal := balRes.Balance
				Expect(vestAccFinalBal.Amount).To(Equal(vestAccInitialBal.Balance.Amount.Add(vestCoinsAmt)))
			})
			...
			It(fmt.Sprintf("should fund the vesting account from a smart contract when defining only lockup (%s)", callType.name), func() {
				...
				// fundCoins := sdk.NewCoins(sdk.NewCoin(s.bondDenom, math.NewInt(1_000)))
				// err = s.factory.FundAccount(s.keyring.GetKey(2), contractAddr.Bytes(), fundCoins)
				// Expect(err).ToNot(HaveOccurred(), "error while funding the contract: %v", err)

				txSenderInitialBal, err := s.grpcHandler.GetBalance(funderKey.AccAddr, s.bondDenom)
				Expect(err).To(BeNil())
				vestAccInitialBal, err := s.grpcHandler.GetBalance(vestingAccountKey.AccAddr, s.bondDenom)
				Expect(err).To(BeNil())
				contractInitialBal, err := s.grpcHandler.GetBalance(sdk.AccAddress(contractAddr.Bytes()), s.bondDenom)
				Expect(err).To(BeNil())
				...
				vestAccFinalBal, err := s.grpcHandler.GetBalance(vestingAccountKey.AccAddr, s.bondDenom)
				Expect(err).To(BeNil())
				vestCoinsAmt := math.NewIntFromBigInt(defaultPeriods[0].Amount[0].Amount)
				Expect(vestAccFinalBal.Balance.Amount).To(Equal(vestAccInitialBal.Balance.Amount.Add(vestCoinsAmt)), "expected vesting account to have initial balance plus vesting")

				contractFinalBal, err := s.grpcHandler.GetBalance(sdk.AccAddress(contractAddr.Bytes()), s.bondDenom)
				Expect(err).To(BeNil())
				Expect(contractFinalBal.Balance.Amount).To(Equal(contractInitialBal.Balance.Amount), "expected the contract to have same balance")

				txSenderFinalBal, err := s.grpcHandler.GetBalance(funderKey.AccAddr, s.bondDenom)
				Expect(err).To(BeNil())
				Expect(txSenderFinalBal.Balance.Amount).To(Equal(txSenderInitialBal.Balance.Amount.Sub(fees).Sub(vestCoinsAmt)), "expected the funder to have initial balance less (fee + vesting)")
			})
