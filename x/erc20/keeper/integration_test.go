		...
		Context("with existing coins", func() {
			...
			Describe("for a single Cosmos Coin", func() {
				BeforeEach(func() {
					...
					_, err = testutil.Delegate(s.ctx, s.app, privKey, sdk.NewCoin("aakk", sdk.NewInt(500000000000000000)), s.validator)
					s.Require().NoError(err)
